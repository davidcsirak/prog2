...
public:
    ...
    BinTree(const BinTree & old) {
        ...
        root = cp(old.root, old.treep);           //létrehozom az új fát és átadom a régi fa gyökerét és a fa pointerét is
        
    }
    
    Node * cp(Node *node, Node *treep) 
    {
        Node * newNode = nullptr;  //ha a masolando csp. null
        
        if(node)
        {
            newNode = new Node(node->getValue());      //lemasolom az értéket
            
            newNode->leftChild(cp(node->leftChild(), treep));       //lemásolom a gyerekeket
            newNode->rightChild(cp(node->rightChild(), treep));     
            
            if(node == treep)      //ha a fa mutató az aktuális csp.-ra mutat akkor a mutatót átteszem az új csp.-ra (lemásolom a szituációt)
                this->treep = newNode;
        }
    ...
    BinTree & operator=(const BinTree & old) {
        ...
        BinTree tmp{old};  //létrehozok egy fát és belemásolom a régit
        std::swap(*this, tmp);    //megcserélem a régit az újjal ezáltal a *this-ben lesz a régi amit majd a destruktor töröl
        return *this;
    }
    ...
    BinTree(BinTree && old) {
        ...
        root = nullptr;       //az új(üres) fa gyökerét állítom NULL-ra mivel ez majd a régi fa gyökere lesz az értékadástól tehát a swaptól
        *this = std::move(old); //az újba átteszem a régit értékadással tehát meghívódik a swap amivel sikerül törölnöm a régi adatait
    }
       
    BinTree & operator=(BinTree && old) {
        std::cout << "BT move assign" << std::endl;
        
        std::swap(old.root, root);  //a réginek a gyökerét cserélje meg az újnak a gyökerével
        std::swap(old.treep, treep); //a réginek a fa mutatóját cserélje meg az újnak a fa mutatójával
...
int main(int argc, char** argv, char ** env)
{
    ...
    ZLWTree<char, '/', '0'> zt4 = std::move(zt2);   //a zt2-t teszem a zt4be (rootját)
